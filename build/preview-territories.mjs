...
const panels = YEARS.map((year) => {
  ...
  <svg viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" aria-label="Territory zones in ${Math.abs(year)} BCE">
    <rect width="${W}" height="${H}" fill="#eef2f3" />
${shapes}
${labels}
  </svg>
</section>`;
...
<p class="note">Exactly the polygons, palette, and opacities the map draws, on a plain equirectangular grid with no basemap. Shapes are deliberately schematic zones of control, not surveyed borders. Faded zones are non-belligerent context powers.</p>
${panels}
`, "utf8");
