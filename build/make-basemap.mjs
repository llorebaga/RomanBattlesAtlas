...
const output = { type: "FeatureCollection", features: [{ type: "Feature", properties: {}, geometry: { type: "MultiPolygon", coordinates: polygons } }] };
mkdirSync(new URL("../data/geo/", import.meta.url), { recursive: true });
const target = new URL("../data/geo/mediterranean-land.json", import.meta.url);
...
console.log(`wrote ${(JSON.stringify(output).length / 1024).toFixed(1)} KB to data/geo/mediterranean-land.json`);
