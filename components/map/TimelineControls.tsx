  ...
  return (
    <section className="timeline" aria-label="Campaign timeline">
      <div className="timeline-primary">
        <button className="icon-button" onClick={() => onYearChange(WAR_START_YEAR)} aria-label="Return to 264 BCE"><RotateCcw size={16} /></button>
      ...
      <div className="slider-wrap">
        ...
        <input aria-label="Selected year, from 264 to 241 BCE" type="range" min={WAR_START_YEAR} max={WAR_END_YEAR} step={1} value={year} onChange={(event) => onYearChange(Number(event.target.value))} style={{ "--timeline-progress": `${((year - WAR_START_YEAR) / (WAR_END_YEAR - WAR_START_YEAR)) * 100}%` } as React.CSSProperties} />
        <span aria-hidden="true">241 BCE</span>
      </div>
