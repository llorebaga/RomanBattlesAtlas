...
export default async function BattlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="battle-page">
      <nav className="detail-nav"><Link href="/map"><ArrowLeft size={16} /> Campaign map</Link><Link href="/" className="detail-brand">ROMAN CAMPAIGN ATLAS</Link><span>FIRST PUNIC WAR</span></nav>
      <header className="battle-hero">
        <div className="hero-gridline" aria-hidden="true" />
        <div className="battle-hero-content">
          <div className="battle-hero-kicker"><span>{battle.kind === "naval" ? "Naval engagement" : battle.kind}</span><span>First Punic War</span></div>
          <h1>{battle.name}</h1>
      ...
      <div className="battle-content">
        ...
        <article className="battle-article">
          ...
          <section className="uncertainty-section"><span className="section-number">{isMylae ? "08" : "02"}</span><div><p className="eyebrow">EVIDENCE REGISTER</p><h2>What remains uncertain</h2><ul>{battle.uncertaintyNotes.map((note) => <li key={note}>{note}</li>)}</ul><p className="method-note">This page separates surviving testimony from modern reconstruction. It does not invent precise tracks, dialogue, weather, or unit positions.</p></div></section>
          <section className="sources-section"><span className="section-number">{isMylae ? "09" : "03"}</span><div><p className="eyebrow">FURTHER READING</p><h2>Sources</h2><div className="sources-grid"><SourceList title="Ancient testimony" ids={battle.ancientSourceIds} /><SourceList title="Modern studies" ids={battle.modernSourceIds} /></div></div></section>
        </article>
      </div>
