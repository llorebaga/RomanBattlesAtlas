...
import { getBattleDiagram, NO_DIAGRAM_REASON } from "@/data/battleDiagrams";
import { BattleDiagram } from "@/components/battles/BattleDiagram";

export function generateStaticParams() { return battles.map((battle) => ({ slug: battle.slug })); }

// The side of a force estimate is prose ("Rome", "Macedon", "Saguntum"), so the
// swatch has to be looked up rather than guessed. Anything unrecognised falls back
// to a neutral tone instead of silently claiming a Carthaginian force.
...
function sideColor(side: string): string {
  ...
  return faction ? factionColor(faction) : "#6c6b61";
}
...
export default async function BattlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const diagram = getBattleDiagram(battle.slug);
  const uncertaintyNumber = isMylae ? "07" : hasRich ? "06" : "03";
  const sourcesNumber = isMylae ? "08" : hasRich ? "07" : "04";
      ...
      <div className="battle-content">
        ...
        <article className="battle-article">
          {isMylae ? <>
            ...
            <section><span className="section-number">03</span><div><p className="eyebrow">ESTIMATED FORCES</p><h2>Large fleets, uncertain totals</h2><div className="force-estimates">{battle.forces?.map((force) => <div key={force.side}><span className="force-icon" style={{ background: sideColor(force.side) }}><ShipWheel size={20} /></span><h3>{force.side}</h3><strong>{force.estimate}</strong><EvidenceBadge certainty={force.certainty} /><p>{force.note}</p></div>)}</div></div></section>
            <section className="wide-section"><span className="section-number">04</span><div><p className="eyebrow">HOW THE BATTLE DEVELOPED</p><h2>Stage by stage</h2>{diagram ? <BattleDiagram diagram={diagram} title={battle.name} /> : <BattleTimeline moments={battle.moments ?? []} />}</div></section>
            <section><span className="section-number">05</span><div><p className="eyebrow">WHY IT MATTERED</p><h2>A strategic horizon opened</h2><div className="importance-grid"><div><Shield size={22} /><h3>Operational confidence</h3><p>Rome demonstrated that it could contest coastal sea lanes and protect operations in Sicily.</p></div><div><Swords size={22} /><h3>Boarding as adaptation</h3><p>The victory showed how Roman infantry practice could be transferred to naval combat—at least in favorable circumstances.</p></div><div><ShipWheel size={22} /><h3>Not a final solution</h3><p>Later storms and defeats underline that a single success did not erase Carthage’s naval experience.</p></div></div></div></section>
            <section><span className="section-number">06</span><div><p className="eyebrow">LOSSES</p><h2>Reported, not audited</h2><div className="casualty-table">{battle.casualties?.map((item) => <div key={item.side}><strong>{item.side}</strong><span>{item.estimate}</span><EvidenceBadge certainty={item.certainty} /><small>{item.note}</small></div>)}</div></div></section>
          </> : hasRich ? <>
            ...
            {battle.forces && <section><span className="section-number">03</span><div><p className="eyebrow">ESTIMATED FORCES</p><h2>Reported strengths, uncertain totals</h2><div className="force-estimates">{battle.forces.map((force) => <div key={force.side}><span className="force-icon" style={{ background: sideColor(force.side) }}><Swords size={20} /></span><h3>{force.side}</h3><strong>{force.estimate}</strong><EvidenceBadge certainty={force.certainty} />{force.note && <p>{force.note}</p>}</div>)}</div></div></section>}
            <section className="wide-section"><span className="section-number">04</span><div><p className="eyebrow">HOW THE BATTLE DEVELOPED</p><h2>Stage by stage</h2>{diagram ? <BattleDiagram diagram={diagram} title={battle.name} /> : battle.moments ? <BattleTimeline moments={battle.moments} /> : <p className="diagram-absent">{NO_DIAGRAM_REASON[battle.slug] ?? "No tactical reconstruction is offered for this engagement."}</p>}</div></section>
            ...
            <section className="wide-section"><span className="section-number">02</span><div><p className="eyebrow">HOW THE BATTLE DEVELOPED</p><h2>Stage by stage</h2>{diagram ? <BattleDiagram diagram={diagram} title={battle.name} /> : <p className="diagram-absent">{NO_DIAGRAM_REASON[battle.slug] ?? "No tactical reconstruction is offered for this engagement: the surviving accounts do not support one."}</p>}</div></section>
          </>}
